...
bool params::enforce(){
  ...
  //first check whether there are any parameters that cannot be understood

  for(unsigned int i=0; i<par_names.size(); i++){
    string cur_par_name = par_names[i];
    bool valid_param = false;
    for(unsigned int j=0; j<required_params.size(); j++){
      if(cur_par_name == required_params[j]){
	valid_param = true; 
      }
    }

    for(unsigned int j=0; j<optional_params.size(); j++){
      if(cur_par_name == optional_params[j]){
	valid_param = true; 
      }
    }

    if(!valid_param && cur_par_name != "executable"){
      std::cout<<"Could not understand param: "<<cur_par_name<<std::endl;
      exit(1);
    }
  }
  ...
  for(unsigned int i=0; i<optional_params.size(); i++){
    ...
    if(!param_found){
      std::cout<<"    Optional:\t"<<cur_param<<" = <"<<cur_legend<<">\t"<<stringify_type(cur_type)<<" [ default: "<<cur_default<<" ]\n";
    }
    else{ //type_check now
      string cur_par_value = par_values.at(param_pos);
	...
	all_enforced = false;
	std::cout<<"    Optional:\t"<<cur_param<<" = <"<<cur_legend<<">\t"<<stringify_type(cur_type)<<" [ unexpected type ]\n";
      } else {
	if(!all_enforced){
	  std::cout<<"    Optional:\t"<<cur_param<<" = <"<<cur_legend<<">\t"<<stringify_type(cur_type)<<" [ default: "<<cur_default<<" ]\n";
	}
...
void params::set_value(const string& par_name, const string& par_value){
  int existing_ind = -1;
  for(unsigned int i=0; i<par_names.size(); i++){
    if(par_names[i] == par_name){
      existing_ind = i;
    }
  }
  if(existing_ind != -1){
    par_names.erase(par_names.begin() + existing_ind);
    par_values.erase(par_values.begin() + existing_ind);
  }
  par_names.push_back(par_name);
  par_values.push_back(par_value);
}
